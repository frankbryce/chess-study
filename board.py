...
class Board:
    ...
    def __init__(self):
        self.R = np.zeros(shape=(8,8))
        self.N = np.zeros(shape=(8,8))
        self.B = np.zeros(shape=(8,8))
        self.K = np.zeros(shape=(8,8))
        self.Q = np.zeros(shape=(8,8))
        self.P = np.zeros(shape=(8,8))
        self.r = np.zeros(shape=(8,8))
        self.n = np.zeros(shape=(8,8))
        self.b = np.zeros(shape=(8,8))
        self.k = np.zeros(shape=(8,8))
        ...
        self.turn = _WHITE
        self.castle = np.zeros(shape=(2,2))
        self.enpassant = np.zeros(shape=(8,8))
        self.halfmove = 0
        self.fullmove = 0

    def load_fen(self, fen):
        row_re = r'[12345678rnbkqpRNBKQP]+'
        turn_re = r'[wb]'
        castle_re = r'[KQkq-]+'
        enpassant_re = r'-|[abcdef][123456678]'
        move_re = r'[0-9]+'
        fen_re = re.compile(rf'({row_re})/({row_re})/({row_re})/({row_re})/'
                            rf'({row_re})/({row_re})/({row_re})/({row_re})\s+'
                            rf'({turn_re})\s+({castle_re})\s+({enpassant_re})\s+'
                            rf'({move_re})\s+({move_re})')
        match = fen_re.match(fen)
        if not match:
            raise ValueError(f'Could not parge FEN notation: {fen}')
        rows = match.group(8,7,6,5,4,3,2,1)
        turn = match.group(9)
        castle = match.group(10)
        enpassant = match.group(11)
        halfmove = match.group(12)
        fullmove = match.group(13)

        for i,row in enumerate(rows):
            col = 0
            for char in row:
                if col > 7:
                    raise ValueError(f'FEN notation has more than 8 squares in row {i}: {fen}')
                if char.isnumeric():
                    col += int(char)
                    continue
                try:
                    ar = getattr(self, char)
                    ar[i][col] = 1
                except e:
                    raise ValueError(f'FEN notation had invalid piece, "{char}": {fen}') from e
                col += 1
        ...
        for char in castle:
            if char == 'K':
                self.castle[_WHITE][_CASTLE_SHORT] = 1
            elif char == 'Q':
                self.castle[_WHITE][_CASTLE_LONG] = 1
            elif char == 'k':
                self.castle[_BLACK][_CASTLE_SHORT] = 1
            elif char == 'q':
                self.castle[_BLACK][_CASTLE_LONG] = 1
            elif char == '-':
                continue
            else:
                raise ValueError(f'could not parse castle part of FEN: {fen}')
        
        if enpassant != '-':
            if len(enpassant) != 2:
                raise ValueError(f'enpassant square must be 2 characters in FEN: {fen}')
            try:
                col = list('abcdefgh').index(enpassant[0])
                row = int(enpassant[1])
                enpassant[row][col] = 1
            except e:
                raise ValueError(f'could not parse enpassant square in FEN: {fen}') from e

        if not halfmove.isnumeric():
            raise ValueError(f'halfmove was not numeric in FEN: {fen}')
        self.halfmove = int(halfmove)

        if not fullmove.isnumeric():
            raise ValueError(f'fullmove was not numeric in FEN: {fen}')
        self.fullmove = int(fullmove)

        return self

    def white_sum(self):
        return self.P + self.R + self.N + self.B + self.K + self.Q
    ...
    def get_fen(self):
        ...
        for row in [7,6,5,4,3,2,1,0]:
            ...
            while col < 8:
                blanks = 0
                while col < 8 and board_sum[row][col] == 0:
                    blanks += 1
                    col += 1
                if blanks != 0:
                    fen += f'{blanks}'
                if col < 8:
                    fen += f'{self.piece(row,col)}'
                col += 1
            if row != 0:
                fen += '/'
        ...
        ### TURN ###
        fen += ' '
        fen += ('w' if self.turn == _WHITE else 'b')

        ### CASTLES ###
        fen += ' '
        castles = False
        if np.sum(self.castle) == 0:
            fen += '-'
        else:
            if self.castle[_WHITE][_CASTLE_SHORT]:
                fen += 'K'
            if self.castle[_WHITE][_CASTLE_SHORT]:
                fen += 'Q'
            if self.castle[_BLACK][_CASTLE_LONG]:
                fen += 'k'
            if self.castle[_BLACK][_CASTLE_LONG]:
                fen += 'q'
        ...
        ### EN PASSANT ###
        fen += ' '
        if np.sum(self.enpassant) == 0:
            fen += '-'
        else:
            fen += str(np.where(self.enpassant == 1))
...
if __name__ == '__main__':
    ...
    for fen in fens:
        ...
        assert Board().load_fen(fen).get_fen() == fen
